...
Verification completed on 2026-07-15. The focused fnapp tests, repository script suites, `go test ./... -count=1`, `go build ./...`, and the real FPK build passed. The first packaging run exposed a pre-existing race in the repository-wide Go source scan when ignored runtime YAML files disappeared concurrently; commits `b96aa11`, `0e573db`, and `c7828fc` added focused coverage and limited the scanner exception to missing regular files with known runtime configuration names, while missing directories, Go files, unrelated files, unknown entry types, and other errors still fail. The final package is version `0.0.2`, SHA-256 `8C6107C0889FD6D7E7BBE06AB9C6C0051F7120467657C6206F138F087172AA10`, and contains configuration only under `server/defaults/` inside `app.tgz`.
