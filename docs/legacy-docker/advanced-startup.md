...
完全的环境变量列表，可以直接查看当前仓库中的 `cmd/` 与 `config/model/` 实现。


## 命令行参数

```
docker run --rm -it -p 5005:5005 junfuchang/superflare:0.5.1 superflare
找不到配置文件config，创建默认配置。
[22:49:49.562] INFO: SuperFlare - 🏂 Challenge all bookmarking apps and websites directories, Aim to Be a best performance monster.
...
启用方法，在 superflare 启动参数后添加 `-m=1` 或者设置环境变量 `FLARE_MINI_REQUEST=1`

### 免登陆模式：`disable_login`

当这个参数关闭之后，用户需要在登陆之后，才能够对设置页面的内容进行调整，适合 SuperFlare 在公网环境中使用的小伙伴。

此参数默认开启，方便在 HomeLab 或本地使用的小伙伴，减少不必要的操作。

禁用方法，在 superflare 启动参数后添加 `--disable_login=0`

### 离线模式：`offline`

当这个参数开启之后，superflare 所有依赖公网的功能将被停用。

目前 superflare 在相关功能开启时会访问天气和地理位置接口，开启此参数后，天气将无法获取。

此参数默认关闭，开启方法，在 superflare 启动参数后添加：`--offline=1`

### 指定端口：`port`

这个参数用于自定义服务端口，使用历史 Docker 运行方式的场景可以忽略。
